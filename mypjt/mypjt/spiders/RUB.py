...
class RubSpider(scrapy.Spider):
    ...
    start_urls = ['http://www.zou114.com/agiotage/hl2.asp?from=RUB&to=CNY&q=1']
    def __init__(self):
        super(RubSpider,self).__init__()
        ...
        self.exchange_currency_list=['CNY','JPY','GBP','EUR','USD']#人民币 美元 英镑 欧元 卢布
        self.index=0
    ...
    def parse(self, response):
        ...
        if  self.index<len(self.exchange_currency_list):
            new_url='http://www.zou114.com/agiotage/hl2.asp?from=RUB&to={0}&q=1'.format(self.exchange_currency_list[self.index])
            logger.info(new_url)
